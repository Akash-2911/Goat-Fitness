...
const facts = [
  {
    icon: Flame,
    fact: "You burn calories even while sleeping — about 50 cal per hour.",
    color: "text-orange-400",
    bg: "bg-orange-400/10",
  },
  {
    icon: Droplets,
    fact: "Drinking water before meals can reduce calorie intake by up to 13%.",
    color: "text-blue-400",
    bg: "bg-blue-400/10",
  },
  {
    icon: Target,
    fact: "It takes 21 days to form a habit and 90 days to make it a lifestyle.",
    color: "text-[#C8FF00]",
    bg: "bg-[#C8FF00]/10",
  },
];
      ...
      try {
        ...
        data.forEach((goal) => {
          if (goal.type === "calories") {
            setCalorieGoal(Number(goal.target_value));
          }
          if (goal.type === "weight") {
            setWeightGoal(Number(goal.target_value));
          }
          if (goal.type === "water") {
            setWaterGoal(Number(goal.target_value));
          }
        });
      } catch (error) {
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 pt-24 pb-24 md:pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          {facts.map((item, index) => (
            <div
              key={index}
              className="bg-[#111318] border border-white/8 rounded-2xl p-4"
            >
              <div
